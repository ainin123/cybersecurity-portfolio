...
const publications = [
  ...
  { type: "blog", title: "Uncovering the TTPs Behind SCATTERED SPIDER's Social Engineering Playbook", venue: "ANIQA Intelligence Blog", date: "2024-02", tags: ["Social Engineering", "SIM Swapping", "BEC"] },
...
export default function PublicationsSection() {
  ...
  return (
    <section id="publications" ref={ref} className="relative py-28 overflow-hidden" style={{ backgroundColor: "#d1d5db" }}>
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        >
          {Object.entries(typeConfig).map(([type, cfg]) => (
            <span key={type} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono rounded-sm" style={{ color: "#4b5563", border: "1px solid rgba(0,0,0,0.12)", backgroundColor: "#ffffff" }}>
          ...
          {publications.map((pub, i) => {
            ...
            return (
              <motion.div
                ...
                className="p-5 rounded-sm group transition-all hover:shadow-sm"
              style={{ backgroundColor: "#ffffff", border: "1px solid rgba(0,0,0,0.08)" }}
              >
                <div className="flex items-start gap-4">
                  <div className="p-2 rounded-sm shrink-0 mt-0.5" style={{ backgroundColor: "rgba(0,255,136,0.08)", border: "1px solid rgba(0,255,136,0.25)" }}>
                    <cfg.icon className="w-3.5 h-3.5 text-cyber-accent" />
                  </div>

                  <div className="flex-1 min-w-0">
                    <h3 className="font-mono text-sm font-semibold group-hover:text-cyber-accent transition-colors leading-snug mb-2" style={{ color: "#111111" }}>
                      {pub.title}
                    </h3>
                    <div className="flex flex-wrap items-center gap-3 text-xs font-mono mb-2.5" style={{ color: "#6b7280" }}>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {pub.tags.map((tag) => (
                        <span key={tag} className="text-xs font-mono px-2 py-0.5 rounded-sm" style={{ color: "#4b5563", backgroundColor: "#f3f4f6", border: "1px solid rgba(0,0,0,0.08)" }}>
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <a href="#" className="shrink-0 p-2 rounded-sm transition-colors hover:text-cyber-accent" style={{ color: "#9ca3af", border: "1px solid rgba(0,0,0,0.1)" }}>
        ...
        <motion.div
          ...
          className="mt-8 grid grid-cols-2 md:grid-cols-4 rounded-sm overflow-hidden"
        ...
        >
          {[
            ...
            <div key={s.label} className="p-6 text-center" style={{ backgroundColor: "#ffffff", borderRight: "1px solid rgba(0,0,0,0.06)" }}>
              <div className="text-2xl font-mono font-bold text-cyber-accent mb-1">{s.value}</div>
              <div className="text-xs font-mono tracking-wider" style={{ color: "#6b7280" }}>{s.label}</div>
            </div>
          ))}
        </motion.div>
      </div>
